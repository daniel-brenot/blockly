...
const ConstantProvider = function() {
   ...
   * @package
   */
  this.FIELD_BORDER_RECT_COLOR = '#fff';
   ...
   * @type {boolean}
   */
  this.FIELD_DROPDOWN_COLORED_DIV = false;
   ...
   * @type {boolean}
   */
  this.FIELD_COLOR_FULL_BLOCK = false;

  /**
   * A color field's default width.
   * @type {number}
   */
  this.FIELD_COLOR_DEFAULT_WIDTH = 26;

  /**
   * A color field's default height.
   * @type {number}
   */
  this.FIELD_COLOR_DEFAULT_HEIGHT = this.FIELD_BORDER_RECT_HEIGHT;
   ...
   * @package
   */
  this.INSERTION_MARKER_COLOR = '#000000';
...
ConstantProvider.prototype.setComponentConstants_ = function(theme) {
  this.CURSOR_COLOR =
      theme.getComponentStyle('cursorColor') || this.CURSOR_COLOR;
  this.MARKER_COLOR =
      theme.getComponentStyle('markerColor') || this.MARKER_COLOR;
  this.INSERTION_MARKER_COLOR =
      theme.getComponentStyle('insertionMarkerColor') ||
      this.INSERTION_MARKER_COLOR;
 ...
 * @package
 */
ConstantProvider.prototype.getBlockStyleForColor = function(color) {
  const name = 'auto_' + color;
  if (!this.blockStyles[name]) {
    this.blockStyles[name] = this.createBlockStyle_(color);
  }
...
ConstantProvider.prototype.getBlockStyle = function(blockStyleName) {
  return this.blockStyles[blockStyleName || ''] ||
      (blockStyleName && blockStyleName.indexOf('auto_') === 0 ?
           this.getBlockStyleForColor(blockStyleName.substring(5)).style :
 ...
 * @protected
 */
ConstantProvider.prototype.createBlockStyle_ = function(color) {
  return this.validatedBlockStyle_({'colorPrimary': color});
};
...
ConstantProvider.prototype.validatedBlockStyle_ = function(blockStyle) {
  ...
  const parsedColor =
      parsing.parseBlockColor(valid['colorPrimary'] || '#000');
  valid.colorPrimary = parsedColor.hex;
  valid.colorSecondary = valid['colorSecondary'] ?
      parsing.parseBlockColor(valid['colorSecondary']).hex :
      this.generateSecondaryColor_(valid.colorPrimary);
  valid.colorTertiary = valid['colorTertiary'] ?
      parsing.parseBlockColor(valid['colorTertiary']).hex :
      this.generateTertiaryColor_(valid.colorPrimary);
 ...
 * @protected
 */
ConstantProvider.prototype.generateSecondaryColor_ = function(inputColor) {
  return color.blend('#fff', inputColor, 0.6) || inputColor;
};
 ...
 * @protected
 */
ConstantProvider.prototype.generateTertiaryColor_ = function(inputColor) {
  return color.blend('#fff', inputColor, 0.3) || inputColor;
};
